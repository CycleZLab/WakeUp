...
//@Disabled
@TeleOp
public class ForwardVelo extends LinearOpMode {
...
//            drivetrain.drive(gamepad1);
            ...
            if (!end) {
                if (Math.abs(drivetrain.pp.getPosX(DistanceUnit.INCH)) > DISTANCE) {
                    end = true;
                    drivetrain.setTeleOpDrive(0, 0, 0);
                } else {
                    drivetrain.setTeleOpDrive(1, 0, 0);
                    //double currentVelocity = Math.abs(follower.getVelocity().getXComponent());
                    double currentVelocity = Math.abs(drivetrain.pp.getVelX(DistanceUnit.INCH));
